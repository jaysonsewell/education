...
int InsertionSort(int the_array[], unsigned int size) {
  ...
  for (unsigned int i = 0; i < size; i++) {
    int a = i;
    num++;
    while (a > 0 && the_array[a] < the_array[a - 1]) {
      SwapValues(the_array[a], the_array[a - 1]);
      a = a - 1;
    }
  }
  return num;
  ...
  int gap = size / 2;
    while (gap > 0) {
      for (unsigned int i = gap; i < size; i++) {
        ...
        int a = i;
        while (a >= gap && the_array[a - gap] > temp) {
          SwapValues(the_array[a], the_array[a - gap]);
          // the_array[a] = the_array[a - gap];
          a = a - gap;
        }
        the_array[a] = temp;
      }
